...
[[ "$TAXI_ACCESS_ENABLED" == "true" || "$TAXI_ACCESS_ENABLED" == "false" ]] || {
  echo 'TAXI_ACCESS_ENABLED must be true or false.' >&2
  exit 6
}
[[ "$TAXI_TRIPS_ENABLED" == "false" ]] || {
  echo 'Taxi trips remain candidate-only and must stay disabled in Production canonical schema 034.' >&2
  exit 6
}
...
gcloud builds submit . \
  ...
  --substitutions "COMMIT_SHA=${COMMIT_SHA},_REGION=${GOOGLE_CLOUD_REGION},_AR_REPOSITORY=${OPERATIONS_ARTIFACT_REPOSITORY},_BACKEND_SERVICE=${OPERATIONS_BACKEND_SERVICE},_FRONTEND_SERVICE=${OPERATIONS_FRONTEND_SERVICE},_RUNTIME_SERVICE_ACCOUNT=${OPERATIONS_RUNTIME_SERVICE_ACCOUNT},_CLOUD_SQL_INSTANCE=${CLOUD_SQL_INSTANCE_CONNECTION_NAME},_GCS_MEDIA_BUCKET=${GCS_MEDIA_BUCKET},_CORS_ORIGIN=${CORS_ORIGIN},_SITE_URL=${NEXT_PUBLIC_SITE_URL},_EMAIL_FROM=${EMAIL_FROM},_GOOGLE_LOGGING_ENABLED=${GOOGLE_LOGGING_ENABLED},_GOOGLE_MONITORING_ENABLED=${GOOGLE_MONITORING_ENABLED},_GOOGLE_ERROR_REPORTING_ENABLED=${GOOGLE_ERROR_REPORTING_ENABLED},_FACEBOOK_AUTH_ENABLED=${FACEBOOK_AUTH_ENABLED},_CLASSIFIEDS_ENABLED=${CLASSIFIEDS_ENABLED},_NEXT_PUBLIC_CLASSIFIEDS_ENABLED=${NEXT_PUBLIC_CLASSIFIEDS_ENABLED},_TAXI_ACCESS_ENABLED=${TAXI_ACCESS_ENABLED},_TAXI_TRIPS_ENABLED=${TAXI_TRIPS_ENABLED}" \
  --quiet
