...
for (const required of ['--update-env-vars="CORS_ORIGIN=${frontend_url}"', 'Origin: ${frontend_url}', 'Cookie: khedmah_session=preview-csrf-probe']) {
  if (!deployment.includes(required)) throw new Error(`Authenticated preview origin validation is missing: ${required}`);
}
if (deployment.includes('CORS_ORIGIN=*')) throw new Error('Preview/staging CORS origin must never use a wildcard');
