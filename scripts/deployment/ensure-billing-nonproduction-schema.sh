...
readonly migration_033_sha256='acc42c10459b0f90c276e843942bc712283616fc71ac854c177bb20defa34463'
readonly migration_033_git_blob='7bbdd79c5c129f41134c10ba15eedc644e396888'
...
if [[ "$mode" == 'off' ]]; then echo 'Billing migrations 030 and 033 operation is off.'; exit 0; fi
if [[ "$mode" == 'apply' ]]; then
...
migration_033='backend/migrations/versions/033_billing_admin_role.sql'
actual_033_sha256="$(sha256sum "$migration_033" | awk '{print $1}')"
[[ "$actual_033_sha256" == "$migration_033_sha256" ]] || { echo 'Repository Billing migration 033 checksum changed.' >&2; exit 4; }
actual_033_git_blob="$(git hash-object "$migration_033")"
[[ "$actual_033_git_blob" == "$migration_033_git_blob" ]] || { echo 'Repository Billing migration 033 Git blob changed.' >&2; exit 4; }
...
job="khedmah-${environment}-billing-030-033-${short_identifier}"
...
if ! gcloud run jobs execute "$job" --project "$GOOGLE_CLOUD_PROJECT" --region "$GOOGLE_CLOUD_REGION" --wait; then
  echo 'BILLING_030_033_FAILED_EXECUTION' >&2
  echo 'BILLING_030_033_CONTAINER_LOGS_BEGIN' >&2
  ...
  echo 'BILLING_030_033_CONTAINER_LOGS_END' >&2
  exit 1
fi

printf 'Billing migrations 030 and 033 %s completed and verified for %s.\n' "$mode" "$environment"
