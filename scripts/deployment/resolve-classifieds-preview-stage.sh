#!/usr/bin/env bash
set -euo pipefail
stage_file="${1:-.github/classifieds-preview-stage}"
[[ -f "$stage_file" ]] || { echo 'Missing Classifieds Preview stage file.' >&2; exit 2; }
...
case "$stage" in
  ...
  frontend-on)
    ...
    ;;
  *)
    echo "Invalid Classifieds Preview stage: ${stage:-<empty>}" >&2
    exit 2
    ;;
esac
printf 'stage=%s\n' "$stage"
