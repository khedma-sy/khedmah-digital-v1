...
const leaked = contract.split("\n").filter(line => /^[A-Z][A-Z0-9_]+=.+/.test(line) && !/=(?:false|me-central1)$/.test(line));
if (leaked.length) throw new Error(".env.production must not contain populated credentials");
if (production) {
  const missing = required.filter(name => !process.env[name]?.trim());
  if (missing.length) throw new Error(`Missing production environment variables: ${missing.join(", ")}`);
  if (process.env.GCS_MEDIA_LOCATION !== 'me-central1') {
    throw new Error('GCS_MEDIA_LOCATION must be me-central1 for the approved production media stack');
  }
}
