...
test('site atlas names every Web page owner; newly bound launch pages require exact narrow supplements', async () => {
  const [atlas, taxiSupplement, foodSupplement, fulfillmentSupplement] = await Promise.all([
  ...
  assert.deepEqual(
    missingFromAtlas,
    ['apps/frontend/app/food/page.tsx', ...fulfillmentPages, 'apps/frontend/app/taxi/page.tsx'].sort(),
    'no page other than the explicitly supplemented launch and fulfillment pages may bypass the primary atlas'
  );
  ...
  for (const page of fulfillmentPages) {
    assert.ok(fulfillmentSupplement.includes(`](../../${page})`), `Fulfillment supplement must name exact page owner: ${page}`);
  }
  ...
  assert.match(fulfillmentSupplement, /narrow bridge, not a general documentation waiver/i);
