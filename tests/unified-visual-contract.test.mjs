...
test('Web shares compact typography and neutral white glass tokens', async () => {
  ...
  assert.match(tokens, /--k-type-display: clamp\(1\.9rem, 3\.2vw, 2\.8rem\)/);
  assert.match(tokens, /--k-glass: rgb\(255 255 255 \/ 76%\)/);
...
test('Android applies the same compact hierarchy and never crops owner media', async () => {
  ...
  assert.match(theme, /displayLarge = TextStyle\(fontSize = 26\.sp/);
