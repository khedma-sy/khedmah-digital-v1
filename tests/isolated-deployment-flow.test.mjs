...
for(const environment of ['preview','staging'])test(`${environment}: deploys isolated backend first, builds its matching frontend and verifies credentialed CORS`,()=>{
  const f=fixture(environment);try{assert.match(f.run(),/deployment healthy/);const calls=f.calls();const builds=calls.filter(c=>c.tool==='gcloud'&&c.args[0]==='builds');assert.equal(builds.length,6);assert.ok(builds[0].args.includes('cloudbuild.classifieds-migration.yaml'));assert.ok(builds[1].args.includes('cloudbuild.fulfillment-migration.yaml'));assert.ok(builds[2].args.includes('cloudbuild.taxi-pricing-migration.yaml'));assert.ok(builds[3].args.includes('cloudbuild.billing-migration.yaml'));assert.ok(builds[4].args.includes(`cloudbuild.${environment}-backend.yaml`));assert.ok(builds[5].args.some(a=>a.includes(`_NEXT_PUBLIC_API_URL=https://backend-${environment}.example.run.app`)));
