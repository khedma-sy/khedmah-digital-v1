...
function fixture(environment,overrides={}) {
  ...
  const env={...process.env,PATH:bin+':'+process.env.PATH,FIXTURE_LOG:log,FIXTURE_ENV:environment,GOOGLE_CLOUD_PROJECT:`${environment}-fixture`,PRODUCTION_GOOGLE_CLOUD_PROJECT:'production-fixture',GOOGLE_CLOUD_REGION:'me-central1',ARTIFACT_REPOSITORY:`${environment}-images`,RUNTIME_SERVICE_ACCOUNT:'fixture-deployer',CLOUD_SQL_INSTANCE_CONNECTION_NAME:`${environment}-fixture:me-central1:isolated-db`,...overrides};
...
for(const environment of ['preview','staging'])test(`${environment}: deploys isolated backend first, builds its matching frontend and verifies credentialed CORS`,()=>{
  const f=fixture(environment);try{assert.match(f.run(),/deployment healthy/);const calls=f.calls();const builds=calls.filter(c=>c.tool==='gcloud'&&c.args[0]==='builds');assert.equal(builds.length,2);assert.ok(builds[0].args.includes(`cloudbuild.${environment}-backend.yaml`));assert.ok(builds[1].args.some(a=>a.includes(`_NEXT_PUBLIC_API_URL=https://backend-${environment}.example.run.app`)));
    const deploys=calls.filter(c=>c.tool==='gcloud'&&c.args[1]==='deploy');assert.equal(deploys.length,2);const backend=deploys.find(c=>c.args[2].includes('backend'));assert.ok(backend.args.includes('--set-secrets=DATABASE_URL=DATABASE_URL:latest'));assert.ok(backend.args.includes(`${environment}-fixture:me-central1:isolated-db`));
    const origin=calls.find(c=>c.tool==='gcloud'&&c.args[1]==='services'&&c.args[2]==='update');assert.ok(origin.args.includes(`--update-env-vars=CORS_ORIGIN=https://frontend-${environment}.example.run.app`));assert.ok(calls.indexOf(origin)>calls.indexOf(deploys[1]));
