...
`infra/iac/bootstrap` now prepares the shared foundation for a fresh isolated environment: required Google APIs, the Cloud Build source bucket, Artifact Registry, PostgreSQL 16 Cloud SQL instance and application database, runtime/deployer/build service accounts, Workload Identity Federation, runtime secret **containers**, the one-time bootstrap-admin secret container, and the minimum IAM needed for deployment/runtime access. It still does not populate secret values, create the application media bucket, apply application migrations, create Firebase app configuration, or deploy Cloud Run services.

The bootstrap API set includes Cloud Run, Cloud Build, Artifact Registry, Secret Manager, Cloud SQL Admin, Identity Toolkit and Cloud Storage. The runtime identity receives `roles/cloudsql.client`; the dedicated build identity receives only the build/runtime impersonation and secret access it needs; the deployer receives read-only Cloud SQL, Service Usage and Cloud Storage bucket-metadata visibility (`roles/storage.bucketViewer`) plus deployment roles. The bucket-viewer role does not grant object read/write access; runtime media-object permissions remain configured on the separately governed private media bucket. Secret **values** continue to be managed outside Terraform.

For an existing project with manually created resources, import/reconcile them before applying Terraform. Do not create duplicate Cloud SQL instances, repositories, service accounts, WIF resources or secret containers, and never overwrite an existing secret value merely to satisfy the bootstrap stack. Before editing the GitHub `staging` environment, run `scripts/deployment/discover-staging-github-config.sh` with the Staging project selected; it is read-only and reports discoverable metadata plus missing Secret Manager versions without reading secret payloads.
