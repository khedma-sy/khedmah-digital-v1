...
The review-queue slice reports pending and over-24-hour counts for business moderation, professional moderation, verification requests, and mobility-document review. Verification counts mirror the human queue by considering only the latest request per entity and excluding deleted targets. Mobility-document counts mirror the current-document workload by considering only the latest private driver-document asset per business/document type; an asset with no decision row is still pending. These counts are operational workload signals only. KORA cannot approve/reject a profile, approve/reject verification, inspect private document evidence, or approve a driver from these metrics.
