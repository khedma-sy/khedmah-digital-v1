...
- Planning and readiness validation use the dedicated `GCS_MEDIA_LOCATION`; planning, readiness validation, and Production deployment use the same `GCS_MEDIA_BUCKET` and `OPERATIONS_RUNTIME_SERVICE_ACCOUNT` inputs, and deployment passes the reviewed bucket to Cloud Build explicitly.
- Provider selections are committed in this stack's own `.terraform.lock.hcl` for reproducible validation and planning.
- CI initializes and validates both the root and isolated media stacks with remote backends disabled; both validations must pass before merge.
