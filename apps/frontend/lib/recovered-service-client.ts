...
const products={
  ...coreApi.products,
  async list(filters:ProductFilters={}){const params=new URLSearchParams();if(filters.q)params.set('q',filters.q);if(filters.categoryCode)params.set('categoryCode',filters.categoryCode);if(filters.cityCode)params.set('cityCode',filters.cityCode);if(filters.businessProfileId)params.set('businessProfileId',filters.businessProfileId);const result=await request<{products:Array<CoreProductListing&{requiresPrescription?:boolean;controlledItem?:boolean}>}>(`/products${params.size?`?${params}`:''}`);return {products:result.products.map(normalizeProduct)};},
  async listMine(){const result=await coreApi.products.listMine();return {products:(result.products as Array<CoreProductListing&{requiresPrescription?:boolean;controlledItem?:boolean}>).map(normalizeProduct)};},
  async get(id:string){const result=await request<{product:CoreProductListing&{requiresPrescription?:boolean;controlledItem?:boolean}}>(`/products/${encodeURIComponent(id)}`);return {product:normalizeProduct(result.product)};},
};
