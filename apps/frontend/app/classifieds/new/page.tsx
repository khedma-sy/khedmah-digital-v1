...
export default function NewClassifiedPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([api.businesses.listMine(), api.classifieds.quota()])
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!current) {
        const created = await api.classifieds.create({ ...payload(), clientRequestId: requestId(CREATE_KEY) });
        current = created.ad;
      } else {
        const updated = await api.classifieds.update(current.id, {
      ...
      if (generation !== lifecycle.current) return;
      setDraft(current);
      const stored = await api.classifieds.listImages(current.id).catch(() => ({ images: [] as AdImage[] }));
      if (generation !== lifecycle.current) return;
      ...
      const submitted = await api.classifieds.submit(current.id, crypto.randomUUID());
      if (generation !== lifecycle.current) return;
      ...
      if (current) {
        try {
          const [latest, stored] = await Promise.all([api.classifieds.getMine(current.id), api.classifieds.listImages(current.id)]);
