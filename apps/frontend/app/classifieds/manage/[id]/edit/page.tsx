...
export default function EditClassifiedPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([classifiedsApi.getMine(id), classifiedsApi.listImages(id), api.businesses.listMine()])
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      let current = (await classifiedsApi.update(ad.id, {
      ...
      if (selected.length) current = (await classifiedsApi.getMine(ad.id)).ad;
  ...
  async function removeImage(image: AdImage) {
    ...
    operation.current = true; setSaving(true); setError(''); setNotice('');
    try {
      const result = await classifiedsApi.deleteImage(ad.id, image.id, { clientRequestId: crypto.randomUUID(), expectedContentRevision: ad.contentRevision });
  ...
  async function deactivate() {
    ...
    try {
      const result = await classifiedsApi.deactivate(ad.id, { clientRequestId: crypto.randomUUID(), expectedRevision: ad.revision });
