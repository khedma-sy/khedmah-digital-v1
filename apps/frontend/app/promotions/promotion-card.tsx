...
export function PromotionCard({promotion}:{promotion:KhedmahPromotion}){
 ...
 const price=(n:number)=>`${n.toLocaleString('ar-SY-u-nu-latn',{maximumFractionDigits:2})} ${currency}`;
 const discount=promotion.discountType==='percentage'?`خصم ${promotion.discountValue.toLocaleString('ar-SY-u-nu-latn')}٪`:`توفير ${price(promotion.discountValue)}`;
 async function claim(){setBusy(true);setResult(null);try{const{claim}=await api.promotions.claim(promotion.id);setResult({tone:'success',text:`رمزك: ${claim.redemptionCode} — صالح حتى ${new Date(claim.expiresAt).toLocaleString('ar-SY-u-nu-latn')}`})}catch(c){const e=c as Error&{statusCode?:number};setResult({tone:e.statusCode===401?'warning':'danger',text:e.statusCode===401?'سجّل الدخول أولًا لحجز رمز العرض.':e.message||'تعذر حجز العرض.'})}finally{setBusy(false)}}
 return <Surface as="article" className={styles.promotionCard}>
  ...
  <div className={styles.cardMeta}><span><PlatformIcon name="ticket" size={16}/>متبقٍ {available.toLocaleString('ar-SY-u-nu-latn')} رمز</span><span>ينتهي {new Date(promotion.endsAt).toLocaleDateString('ar-SY-u-nu-latn')}</span></div>
