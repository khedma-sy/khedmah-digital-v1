...
export default function ManageBusinessProfilePage() {
  ...
  return <PageShell className={styles.page} label="إدارة النشاط">
    ...
    <nav className={styles.secondaryActions} aria-label="إجراءات الملف"><button type="button" onClick={() => setShowProfileForm((value) => !value)}><PlatformIcon name="briefcase" size={18}/> تعديل المعلومات</button><Link href={`/business-profiles/${id}`}><PlatformIcon name="eye" size={18}/> معاينة الصفحة</Link><Link href="/professional-opportunities"><PlatformIcon name="tools" size={18}/> فرص العمل المهنية</Link><Link href="/store/manage"><PlatformIcon name="grid" size={18}/> إدارة المنتجات</Link><Link href="/business-profiles"><PlatformIcon name="arrow" size={18}/> أنشطتي</Link></nav>
