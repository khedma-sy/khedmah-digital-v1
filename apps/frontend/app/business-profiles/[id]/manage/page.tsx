...
import { api, BusinessBranch, BusinessSocialLink, MediaAsset, OpeningHours, ProviderContactInquiry, PublicBusinessProfile, PublicServiceListing, PublicUserProfile, UploadedMediaAsset, VerificationStatus } from '../../../../lib/api-client';
...
const DRIVER_DOCUMENT_STATUS={pending:'بانتظار تدقيق الأدمن',approved:'معتمدة',rejected:'مرفوضة — ارفع نسخة صحيحة'} as const;
...
export default function ManageBusinessProfilePage() {
  ...
  return <PageShell className={styles.page} label="إدارة النشاط">
    ...
    <Surface className={styles.progress}><div><span>اكتمال الملف</span><strong>{completion.toLocaleString('ar-SY-u-nu-latn')}٪</strong></div><progress max="100" value={completion}>{completion}%</progress><p>أكمل بيانات التواصل والخدمات والساعات والفروع قبل الإرسال للمراجعة.</p></Surface>
    ...
    <div className={styles.workspace}>
      ...
      <Surface className={styles.section}><div className={styles.sectionHeading}><h2>الاستفسارات الواردة</h2><span>{inquiries.length}</span></div>{inquiries.length === 0 ? <p className={styles.empty}>لا توجد استفسارات جديدة.</p> : <div className={styles.list}>{inquiries.map((inquiry) => <article key={inquiry.id} className={styles.inquiry}><div><h3>{inquiry.name}</h3><time dateTime={inquiry.createdAt}>{new Date(inquiry.createdAt).toLocaleDateString('ar-SY-u-nu-latn')}</time></div><p>{inquiry.message}</p><a href={`mailto:${inquiry.contactEmail}`} dir="ltr">{inquiry.contactEmail}</a></article>)}</div>}</Surface>
