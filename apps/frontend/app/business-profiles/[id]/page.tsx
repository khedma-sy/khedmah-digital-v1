...
export default function BusinessProfilePage() {
  ...
  return (
    <main id="foundation-content" className="operations-shell" aria-label={`ملف ${business.name}`}>
      <header className="operations-header">
        <div>
          <p className="eyebrow">{business.categoryCode} · {business.cityCode}</p>
          <h1>{business.name}</h1>
          <span className="status-badge">{business.trustStatus === 'approved' ? '✓ معتمد' : business.trustStatus === 'suspended' ? '✗ موقوف' : '⏳ قيد المراجعة'}</span>
        </div>
