...
export function SmartAssistant() {
  ...
  return <aside className={styles.root} data-khedmah-assistant="true" aria-label="مساعد خدمة ديجتل الذكي" onKeyDown={(event) => {
    if (event.key === 'Escape' && open) { event.preventDefault(); close(); }
  }}>
    <button ref={triggerRef} className={styles.trigger} type="button" onClick={() => { if (open) close(); else setOpen(true); }} aria-label={open ? 'إغلاق مساعد خدمة ديجتل' : 'فتح مساعد خدمة ديجتل'} aria-expanded={open} aria-controls={open ? 'khedmah-assistant-panel' : undefined}>☂ <span>اسأل خدمة ديجتل</span></button>
    {open && <div id="khedmah-assistant-panel" className={styles.panel} role="region" aria-label="طلب المساعدة"><div className={styles.heading}><strong>مساعد خدمة ديجتل</strong><button type="button" onClick={close} aria-label="إغلاق المساعد">×</button></div><p>{message}</p><form onSubmit={submit}><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="مثال: أريد تكسي إلى دمشق" aria-label="طلبك للمساعد"/><div className={styles.actions}><button type="button" onClick={listen} aria-pressed={listening}>{listening ? 'جاري الاستماع…' : '🎙 تحدث'}</button><button type="submit">اعثر عليها</button></div></form><div className={styles.quick}><button onClick={() => go('تكسي')} type="button">تكسي</button><button onClick={() => go('مندوب توصيل')} type="button">مندوب</button><button onClick={() => go('إعلانات')} type="button">إعلانات</button></div><small>لا يتم حفظ التسجيل الصوتي داخل خدمة ديجتل.</small></div>}
  </aside>;
}
