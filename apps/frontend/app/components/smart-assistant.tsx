...
export function SmartAssistant() {
  ...
  return <aside className={`${styles.root} smart-assistant`} aria-label="مساعد خدمة الذكي">
    ...
    <button className={`${styles.trigger} smart-assistant-trigger`} type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? 'إغلاق مساعد خدمة' : 'فتح مساعد خدمة'}><BrandUmbrella className={styles.brandIcon}/><span>اسأل خدمة</span></button>
  </aside>;
}
