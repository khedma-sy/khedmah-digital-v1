...
export default function ProductPage() {
  ...
  const foodMenuEnabled=['restaurant','cafe','bakery','sweets','catering','juice_icecream'].includes(product.categoryCode);
  return <PageShell className={styles.page} label={product.titleAr}><PageHeader eyebrow="إعلان مبوب" title={product.titleAr} description={`معلن بواسطة ${product.businessName??'نشاط على خدمة'}`} backHref="/classifieds"/><div className={styles.detail}><Surface className={styles.gallery}>{images.length?images.map((url,index)=><div className={styles.image} key={url}><img src={url} alt={`${product.titleAr} — صورة ${index+1}`}/></div>):<div className={styles.image}><span>خ</span></div>}</Surface><Surface className={styles.detailBody}><div className={styles.meta}><span className={styles.verified}><PlatformIcon name="check" size={14}/>نشاط موثّق</span><time dateTime={product.createdAt}>نُشر {new Date(product.createdAt).toLocaleDateString('ar-SY-u-nu-latn')}</time></div><span className={styles.status}>{product.availability==='in_stock'?'متوفر':product.availability==='made_to_order'?'حسب الطلب':'غير متوفر'}</span><strong className={styles.price}>{product.price.toLocaleString('ar-SY-u-nu-latn')} {product.currency}</strong>{product.descriptionAr&&<p>{product.descriptionAr}</p>}<div className={styles.actions}>{foodMenuEnabled?<ActionLink href={`/restaurants/${product.businessProfileId}`}>عرض قائمة المطعم واختيار الوجبات</ActionLink>:orderEnabled&&<ActionLink href={`/orders/checkout?productId=${encodeURIComponent(product.id)}`}>اطلب الآن — الدفع نقدي</ActionLink>}<ActionLink href={`/business-profiles/${product.businessProfileId}`} variant={orderEnabled?'secondary':'primary'}>فتح النشاط المعلن</ActionLink><ActionButton type="button" variant="secondary" onClick={share}>واتساب</ActionButton><ActionButton type="button" variant="secondary" onClick={()=>void copy()}>نسخ الرابط</ActionButton></div>{notice&&<p role="status">{notice}</p>}<p className={styles.notice}>{orderEnabled?'يؤكد النشاط الطلب ورسوم التوصيل قبل تعيين المندوب. الدفع نقدي عند التسليم.':'التواصل والاتفاق يتمان مباشرة مع النشاط المعلن.'}</p></Surface></div></PageShell>;
}
