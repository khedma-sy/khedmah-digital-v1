...
export default function EditProductPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const currentImageCount = product?.imageUrls?.length ?? (product?.imageUrl ? 1 : 0);
  ...
  return <PageShell className={styles.page} label="تعديل المنتج"><div className={styles.formShell}>
    ...
    <Surface as="form" className={styles.form} onSubmit={submit}>
      ...
      <label className={styles.field}>إضافة صور (اختياري)<input name="productImage" type="file" accept="image/jpeg,image/png,image/webp" multiple disabled={(product.imageUrls?.length ?? (product.imageUrl ? 1 : 0)) >= 5}/></label>
      <p className={styles.notice}>لديك {product.imageUrls?.length ?? (product.imageUrl ? 1 : 0)} من 5 صور. تبقى الصور الحالية وتُعرض كاملة دون قص.</p>
