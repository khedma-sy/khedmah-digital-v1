...
export class BillingService{
  ...
  async activePromo(token:string|undefined){
    ...
    return {promotion:{code:promo.code,percentageOff:promo.percentage_off,messageAr:promo.message_ar,validUntil:promo.valid_until.toISOString(),eligible:Number(used[0]?.count??0)<promo.per_user_limit}};
  }
  ...
  async markPaid(token:string|undefined,orderIdValue:unknown,value:unknown){
    ...
    return this.db.transaction(async client=>{
      const result=await client.query<OrderRow&{duration_months:number;points_granted:string}>(`SELECT o.*,p.duration_months,p.points_granted::text FROM billing_purchase_orders o JOIN billing_plans p ON p.code=o.plan_code WHERE o.id=$1 FOR UPDATE`,[orderId]);
      const order=result.rows[0];if(!order)throw new NotFoundException('Billing order was not found.');
      if(order.status==='paid'){const existing=await client.query<Record<string,unknown>&{id:string;plan_code:string;period_start:Date;period_end:Date}>(`SELECT id,plan_code,period_start,period_end FROM billing_subscriptions WHERE purchase_order_id=$1`,[order.id]);return {order:mapOrder(order),subscription:existing.rows[0]};}
      ...
      await client.query(`INSERT INTO billing_credit_ledger(id,user_id,grant_id,entry_key,entry_type,points_delta,metadata) VALUES($1,$2,$3,$4,'grant',$5,$6)`,[randomUUID(),order.user_id,grantId,`grant:subscription:${order.id}`,points,JSON.stringify({planCode:order.plan_code,purchaseOrderId:order.id})]);
