...
test('smart admin report exposes bounded product auto-approval results', async () => {
  ...
  const service = new OperationsProductService(identity as never, identityRepository as never, rbac as never, { listIncidents: () => [], listChanges: () => [] } as never, analytics as never);
