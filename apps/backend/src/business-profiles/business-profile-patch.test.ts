...
function fixture(){
  let profile:any={id:'business',ownerUserId:'owner',name:'نشاط سابق',descriptionAr:'وصف',descriptionEn:'Description',phone:'0123456789',email:'owner@example.test',website:'https://example.test',addressAr:'عنوان',lat:33.5,lng:36.3,categoryCode:'repairs',cityCode:'damascus',countryCode:'SY'};
  const repository={findById:async()=>profile,save:async(next:any)=>{profile=next;}} as any;
...
for(const field of ['descriptionAr','descriptionEn','phone','email','website','addressAr']){
  test(`business patch explicitly clears ${field} and preserves omitted data`,async()=>{
    for(const value of ['',null,'   ']){const f=fixture();await f.service.update(undefined,'business',{[field]:value});assert.equal(f.profile[field],undefined);assert.equal(f.profile.name,'نشاط سابق');assert.equal(f.profile.cityCode,'damascus');if(field!=='phone')assert.equal(f.profile.phone,'0123456789');}
  });
}
test('business patch omits absent optional fields and clears a complete coordinate pair',async()=>{
  const f=fixture();await f.service.update(undefined,'business',{name:'اسم جديد'});assert.equal(f.profile.email,'owner@example.test');assert.equal(f.profile.lat,33.5);
  await f.service.update(undefined,'business',{lat:null,lng:null});assert.equal(f.profile.lat,undefined);assert.equal(f.profile.lng,undefined);
});
