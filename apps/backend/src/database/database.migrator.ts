...
export const CANONICAL_SCHEMA_ANCHORS: readonly SchemaAnchor[] = [
  ...
  table('identity', '009', 'identity_credentials'),
  ...['user_identifier', 'email', 'password_hash'].map((name) => column('identity', '009', 'identity_credentials', name)),
  table('business', '004', 'business_profiles'),
  ...['visibility', 'moderation_status', 'trust_status', 'status'].map((name) => column('business', '004', 'business_profiles', name)),
