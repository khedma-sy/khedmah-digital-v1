...
test('canonical schema 025 passes when every contract anchor is present', () => {
  assert.equal(REQUIRED_CANONICAL_SCHEMA_VERSION, '025');
...
for (const scenario of [
  ...
  ['missing 024 product table', (a: (typeof CANONICAL_SCHEMA_ANCHORS)[number]) => a.table === 'product_listings' && a.kind === 'table'],
  ['missing 025 independent classifieds table', (a: (typeof CANONICAL_SCHEMA_ANCHORS)[number]) => a.table === 'ad_listings' && a.kind === 'table'],
  ['missing 025 quota owner constraint', (a: (typeof CANONICAL_SCHEMA_ANCHORS)[number]) => a.name === 'ad_free_slot_owner_fk'],
  ...
  test(`${scenario[0]} fails closed without exposing credentials`, () => {
    assert.throws(() => verifyCanonicalSchema(without(scenario[1])), (error: unknown) => {
      ...
      assert.match(error.message, /CANONICAL_SCHEMA_INCOMPATIBLE required=025 missing=/);
