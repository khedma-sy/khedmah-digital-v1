...
const ALLOWED_OWNER_TYPES: MediaOwnerType[] = ['business_profile', 'professional_profile', 'product_listing', 'ad_listing', 'user'];
const ALLOWED_VISIBILITIES: MediaVisibility[] = ['public', 'private'];
const MAX_SIZE_BYTES = 5 * 1024 * 1024; // 5 MB
const ALLOWED_ASSET_TYPES: MediaAssetType[] = ['logo', 'cover', 'gallery', 'profile_image', 'service_image', 'product_image', 'ad_image'];
...
export function validateUploadMediaRequest(req: UploadMediaRequest): {
  ...
  if (req.ownerType === 'ad_listing' && req.assetType !== 'ad_image') {
    throw new BadRequestException('Classifieds media requires ad_image assetType.');
  }
